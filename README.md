...
### Lexical词法分析器
1. Lexical文件为词法分析器代码文件，代码负责将提供的c-源代码转换为Token序列。
2. Token类定义了Token的结构，包括类型Type、值value、位置row和column。
3. IDKeyWord类定义了关键字和标识符的判断处理。
4. LexicalAnalyzer类负责一行一行读取源代码文件，并通过状态机进行词法分析，输出Token序列。
   * 有参构造指定义输入和输出文件路径。
...
---
* test3.txt为包含语法错误的C-语言代码，包含了变量重复定义错误、使用未声明变量错误和返回语句缺失错误。
* test4.txt为包含语法错误的C-语言代码，包含了函数参数个数不匹配错误、函数返回值类型不匹配错误。
* test5.txt为包含语法错误的C-语言代码，包含了若干语法错误;
* test6.txt为包含词法错误的C-语言代码，出现了错误的字符
